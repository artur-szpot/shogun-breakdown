...
def entered_shop(previous_snapshot: Optional[Snapshot], new_snapshot: Snapshot) -> History:
...
def shop_update(previous_snapshot: Snapshot, new_snapshot: Snapshot) -> History:
    history = previous_snapshot.history
    old_shop = previous_snapshot.shop
    old_service = old_shop.get_service()
    ...
    was_restocked = len(new_shop.items) > len(old_shop.items) \
                    or (old_service is not None and old_service.service == ShopServiceEnum.REROLL_FOR_HP
                        and (new_service is None or new_service.service != ShopServiceEnum.REROLL_FOR_HP))
    ...
    if old_service is not None and new_service is None:
        logger.detail_success(ShopService.service_sale_print(old_service))
        return history
